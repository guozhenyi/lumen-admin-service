...
class UserService
{
    ...
    public function setCaptcha($device, $verifyCode, $timeout = 300)
    {
        ...
        $key = Redis::model()->keyCaptcha($device);
    ...
    public function verifyCaptcha($device, $inputCode)
    {
        ...
        if (empty($inputCode) || empty($knownStr) || $knownStr != $inputCode) {
    ...
    {
        $redis = Base::model()->redis();
        $key = Redis::model()->keyTokenToPayload(sha1($token));
    ...
    {
        $redis = Base::model()->redis();
        $key = Redis::model()->keyUserIdToDevice($user_id);
